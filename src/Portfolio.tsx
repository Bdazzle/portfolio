...
const loadedBackground = (backgroundcolor: string, backgroundimage: string): CSSProperties => {
  return {
    width: `100%`,
    height: `100%`,
...
const animatedBackground = (backgroundcolor: string, backgroundimage: string): CSSProperties => {
  return {
    ...loadedBackground(backgroundcolor, backgroundimage),
...
const BurgerMenu: React.FC<Burger> = ({ barstyle, containerstyle, onClick }) => {
...
/*
pos references Object.keys(icons).length, which doesn't measuer indexes,
...
const navLinkStyle = (textColor: string): CSSProperties => {
  return {
    backgroundClip: 'text',
...
export const Portfolio: React.FC = () => {
  ...
  return (
    ...
    >
      <div className="nested_background"
        onMouseMove={(e) => handleMouseOver(e, 'mouse')}
        onTouchMove={(e) => handleMouseOver(e, 'touch')}
        onMouseUp={() => handleMouseUp()}
        onPointerUp={() => handleMouseUp()}
        style={currentPathname === '/' ? {
          ...animatedBackground(currentTheme.backgroundColor, backgroundimg),
        } : {
          ...loadedBackground(currentTheme.backgroundColor, backgroundimg),
        }
        }
      >
        <Router routes={routes} location={location}>
          ...
          >
            <Link className="nav_link" style={navLinkStyle(currentTheme.color)} to="/"
              onClick={() => setShowNav(false)}
            >Home</Link>
            <Link className="nav_link" style={navLinkStyle(currentTheme.color)} to="/contact"
              onClick={() => setShowNav(false)}
            >Contact</Link>
            <Link className="nav_link" style={navLinkStyle(currentTheme.color)} to="/projects"
              onClick={() => setShowNav(false)}
            >Projects</Link>
            <Link className="nav_link" style={navLinkStyle(currentTheme.color)} to="/about"
              onClick={() => setShowNav(false)}
            >about</Link>
          </div>
